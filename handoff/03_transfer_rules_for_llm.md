...
- `<hh:paraProperties itemCnt="M">` 실제 개수로 M을 기록
  - `<hh:paraPr>`에 `align`, `lineSpacing`, `snapToGrid="0"`, `lineWrap="BREAK"`
  - ‘전 줄바꿈’이 필요한 스타일에는 `<hh:margin><hc:prev value="283" unit="HWPUNIT"/></hh:margin>` 등 space-before 권장
...
- 섹션 프롤로그(컨트롤 전용 문단) 먼저 배치
  ...
  - 페이지 방향은 다음 중 하나로 표현
    - 폭<높이만 유지하고 `landscape` 속성 생략(권장)
    - 또는 `landscape="NARROWLY"` 사용
...
para_items = []
def parapr(pid, align, lsp, space_before=None):
    para_items.append(f'<hh:paraPr id="{pid}" snapToGrid="0">')
